...
2. `JoinAgent._run_async_impl` — barrier that collects dispatched events back into ADK's event stream
3. Fluent builders + IR nodes (that's what adk-fluent IS)
4. `AsyncIterator[str]` → ADK `runner.run_async()` bridge (nobody feeds streams into ADK)
5. Session strategy routing (per_item / shared / keyed)
...
2. `python examples/cookbook/59_dispatch_join.py` — dispatch/join assertions
3. `python examples/cookbook/60_stream_runner.py` — stream assertions
4. `ruff check src/adk_fluent/`
5. `pyright src/adk_fluent/_base.py src/adk_fluent/source.py src/adk_fluent/stream.py`
...
2. What does the execution architecture look like?
3. Is it a separate process? How does the event loop work?
4. What happens with 100 concurrent users × 100 dispatches each?
5. What's missing in Middleware for dispatch/join observability?
6. Design proper ground-up mechanisms, not patch fixes.
...
2. `dict.__setitem__` is **atomic at bytecode level** in CPython
3. Between any two `await` points, code runs to completion without interruption
...
2. `python examples/cookbook/59_dispatch_join.py` — existing dispatch/join assertions still pass
3. `python examples/cookbook/60_stream_runner.py` — existing stream assertions still pass
4. `python examples/cookbook/61_dispatch_middleware.py` — NEW middleware assertions pass
5. `ruff check src/adk_fluent/`
6. `pyright src/adk_fluent/_base.py src/adk_fluent/middleware.py src/adk_fluent/stream.py`