...
1. InstructionProvider reads cached scores, selects top-K events, formats.
...
1. **Dedup**: Remove duplicate events. If within budget, stop.
1. **Truncate lowest priority**: If priority tiers are tagged, drop lowest tier. If within budget, stop.
1. **Summarize**: Summarize the oldest/largest content block. If within budget, stop.
1. **Hard truncate**: If all else fails, truncate oldest content.
...
1. Compact stale tool results → references
1. Compact stale agent outputs → first paragraph + reference
1. Dedup structurally (remove superseded events)
1. Summarize oldest content blocks
1. Hard-truncate as last resort
...
1. `C.select(author=, type=, tag=)` — general event filter
1. `C.recent(decay=, half_life=)` — recency-weighted selection
1. `C.compact(strategy=)` — reversible compaction
1. `C.truncate(max_tokens=, strategy=)` — hard truncation
1. `C.project(fields=, exclude=)` — field projection
1. `C.dedup(strategy="exact"|"structural")` — non-LLM dedup
1. `C.fresh(max_age=, stale_action=)` — freshness filter
1. `C.redact(patterns=)` — PII redaction
1. `C.priority(tier=)` — priority tagging
1. `C.budget(max_tokens=)` — token budget
1. `+` and `|` composition operators with type rules

**Phase 5i.3** (Atoms — LLM-Powered):

1. `C.summarize(scope=, model=, schema=)` — LLM summarization
1. `C.relevant(query_key=, top_k=, model=)` — semantic relevance
1. `C.dedup(strategy="semantic")` — LLM-judged dedup
1. `C.extract(schema=, key=)` — structured extraction
1. `C.distill(key=)` — fact distillation
1. `C.validate(checks=["contradictions"])` — LLM validation
1. `C.fit(target_tokens=, strategy="cascade")` — adaptive fit

**Phase 5i.4** (Scratchpad / Notes):

1. `C.notes(key=, format=)` — read notes
1. `C.write_notes(key=, strategy=)` — write notes
1. Note lifecycle management (merge, consolidate, decay)
1. Note-aware topology inference

**Phase 5i.5** (Molecules — Sugar):

1. `C.rolling(n, summarize=True)` → `C.window(n) + (select_before | summarize)`
1. `C.from_agents_windowed(...)` → per-agent select + window compositions
1. `C.user(strategy=...)` → user select + position filters
1. `C.manus_cascade(budget=)` → compact → dedup → summarize → truncate
