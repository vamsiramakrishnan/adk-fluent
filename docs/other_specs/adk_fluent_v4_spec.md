...
2. **No `__iter__`** or `keys()` — you must use `to_dict()` to enumerate
3. `__contains__` checks both `_value` and `_delta`
4. The `_delta` dict is the same object as `EventActions.state_delta` — writes to state are immediately reflected in the event actions
...
3. Create `InvocationContext` with all services
4. Wrap in OTel trace span
5. Execute via `agent.run_async(ctx)` — yields events
6. For each non-partial event: persist via `session_service.append_event()`
7. Yield events to caller
8. Run event compaction if configured
...
2. State transforms broken → `StateDelta` vs `StateReplacement` (revised below in §6)
3. Inner classes in `build()` → module-level agents
4. Callback merging fragile → compose into single callable
5. Full state copy per function step → read-only view for taps
6. `__init__.py` exports 136 symbols → tiered imports
7. `@agent` decorator monkey-patches → `AgentDecorator` wrapper
8. No "Not Set" vs "Set to None" distinction → `_UNSET` sentinel
9. Generated mega-files → split by logical group
10. `Preset` has no validation → validate against known fields
11. `_clone_shallow` vs `clone()` naming → `_fork_for_operator` vs `clone()`
...
2. **Extracts** field names, types, and defaults
3. **Generates** frozen dataclass IR nodes that mirror the field set
4. **Adds** adk-fluent extensions (`writes_keys`, `reads_keys`, `produces_type`, `consumes_type`)
5. **Re-runs** whenever ADK is updated, producing a diff report
...
2. Generate IR nodes from ADK v1.25.0 models
3. Change `.build()` to return IR
4. Implement ADK backend with generated + hand-written compile
5. Auto-compile in `.ask()`, `.stream()`, `.run()`

### Phase 3: Middleware
1. Protocol aligned with all 13 ADK plugin callbacks
2. `_MiddlewarePlugin` adapter
3. Built-in middleware (retry with error callbacks, circuit breaker, etc.)

### Phase 4: New Capabilities
1. `produces()` / `consumes()` + `check_contracts()`
2. Resource DI with closure injection
3. Event-based testing with `AgentEvent`
4. Graph visualization with data-flow edges
5. ExecutionConfig pass-through (resumability, compaction)
6. ToolConfirmation support
7. Rewind API
...
2. **100-agent pipeline** has type-checked contracts, OTel traces, cost attribution, deterministic tests
3. **ADK version upgrade** requires: run `ir_generator.py`, review diff, update backend if breaking
4. **Existing ADK devs** can adopt incrementally: `to_app()`, `to_runner()` return familiar objects
5. **Middleware → ADK plugin** compilation with correct execution order
6. **1000+ tests, zero LLM calls, <10 seconds**
7. **`pipeline.visualize()`** renders agent graph with data-flow edges
