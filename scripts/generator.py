...
def specs_to_ir_module(specs: list[BuilderSpec], *, manifest: dict | None = None) -> ModuleNode:
    ...
    normalizes type annotations (strip ``types.`` prefix, replace
    unresolvable types with ``Any``), and auto-discovers TYPE_CHECKING
    imports so pyright can resolve every annotation.
    """
    ...
    for spec in specs:
        if not spec.is_composite and not spec.is_standalone:
            already_imported.add(_adk_import_name(spec))
