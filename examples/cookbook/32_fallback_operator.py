...
vector_search = Agent("vector_db").model("gemini-2.0-flash").instruct(
    "Query the vector database for semantically similar documents."
)
fulltext_search = Agent("fulltext_search").model("gemini-2.5-flash").instruct(
    "Perform full-text search across the document corpus with BM25 ranking."
)
...
internal_kb = Agent("internal_kb").model("gemini-2.0-flash").instruct(
    "Search the internal company knowledge base for relevant articles."
)
web_search = Agent("web_search").model("gemini-2.5-flash").instruct(
    "Search the public web for relevant technical documentation."
)
expert_consult = Agent("expert_system").model("gemini-2.5-pro").instruct(
    "Use expert reasoning to synthesize an answer from first principles."
)
...
rag_pipeline = (
    Agent("query_rewriter").model("gemini-2.5-flash")
    .instruct("Rewrite the user query for optimal retrieval.")
    >> (vector_search // fulltext_search)
    >> Agent("answer_generator").model("gemini-2.5-flash")
    .instruct("Generate an answer using the retrieved context.")
)

# Composes with | in parallel — search multiple domains simultaneously
legal_retrieval = (
    Agent("case_law_db").model("gemini-2.0-flash") // Agent("statute_search").model("gemini-2.5-pro")
)
regulatory_retrieval = (
    Agent("reg_db").model("gemini-2.0-flash") // Agent("federal_register").model("gemini-2.5-pro")
)
parallel_legal = legal_retrieval | regulatory_retrieval

# // works with functions too — static fallback for when all LLMs fail
fallback_with_default = Agent("primary_search").model("gemini-2.5-flash").instruct(
