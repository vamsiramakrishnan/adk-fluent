...
from adk_fluent.testing.contracts import check_contracts, _context_description
...
def test_no_dead_key_when_consumed():
    ...
    pipeline = (
        Agent("a").instruct("Classify.").outputs("classification")
        >> Agent("b").instruct("Review the {classification}.")
    )
    ir = pipeline.to_ir()
    issues = check_contracts(ir)
    dead_issues = [i for i in issues if isinstance(i, dict) and "not consumed" in i.get("message", "")]
...
def test_no_dead_key_for_last_agent():
    ...
    pipeline = (
        Agent("a").instruct("Write.") >> Agent("b").instruct("Review.").outputs("final_report")
    )
    ir = pipeline.to_ir()
    issues = check_contracts(ir)
    dead_issues = [i for i in issues if isinstance(i, dict) and "not consumed" in i.get("message", "")]
...
def test_context_description_composite():
    from adk_fluent._context import CWindow, CFromState
...
def test_data_loss_with_user_only_context():
    ...
    pipeline = (
        Agent("a").instruct("Write.")
        >> Agent("b").instruct("Review.").context(C.user_only())
    )
    ir = pipeline.to_ir()
    issues = check_contracts(ir)
    loss_issues = [i for i in issues if isinstance(i, dict) and i.get("level") == "error" and "lost" in i.get("message", "")]
    if loss_issues:
...
def test_data_loss_with_from_agents_including_predecessor():
    ...
    pipeline = (
        ...
        >> Agent("reviewer").instruct("Review.").context(C.from_agents("writer"))
    )
    ir = pipeline.to_ir()
    issues = check_contracts(ir)
    loss_issues = [i for i in issues if isinstance(i, dict) and i.get("level") == "error" and "lost" in i.get("message", "")]
...
def test_data_loss_with_from_agents_excluding_predecessor():
    ...
    pipeline = (
        Agent("writer").instruct("Write.")
        >> Agent("reviewer").instruct("Review.").context(C.from_agents("other_agent"))
    )
    ir = pipeline.to_ir()
    issues = check_contracts(ir)
    loss_issues = [i for i in issues if isinstance(i, dict) and i.get("level") == "error" and "lost" in i.get("message", "")]
