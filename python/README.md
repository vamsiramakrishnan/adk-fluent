...
agent = (
    ...
    .loop_while(lambda s: s.get("quality") != "good", max_retries=3)
)
```
...
| `.context(C.none())`       | `include_contents`   | Control conversation history: `"default"` (full history) or `"none"` (stateless)                              |
...
| `.loop_while(pred, max_retries=3)`      | Retry while `pred(state)` returns True. Inverse of `loop_until`              |
...
pipeline = (
    ...
    >> Agent("formatter", "gemini-2.5-flash").instruct("Format output.").context(C.none())
).build()
```
...
| 07  | Team Coordinator     | LLM-driven delegation with `.delegate_to()`     |
...
| 38  | Retry If             | `.loop_while()` conditional retry      |
...
- **Context control**: `.static()` for cacheable context, `.context(C.none())` for stateless agents, `.prepend()` for dynamic preambles
...
- **v0.16.2** -- Release flow
- **v0.16.1** -- README hero, Release workflow startup failure
...
<a href="https://star-history.com/#vamsiramakrishnan/adk-fluent&Date">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=vamsiramakrishnan/adk-fluent&type=Date&theme=dark">
    <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=vamsiramakrishnan/adk-fluent&type=Date">
    <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=vamsiramakrishnan/adk-fluent&type=Date">
  </picture>
</a>

## Contributors

<a href="https://github.com/vamsiramakrishnan/adk-fluent/graphs/contributors">
  <img alt="Contributors" src="https://contrib.rocks/image?repo=vamsiramakrishnan/adk-fluent">
</a>
...
[MIT](LICENSE) — free for personal and commercial use. Built on top of [Google ADK](https://google.github.io/adk-docs/) (Apache-2.0).

<p align="center">
  <sub>Crafted with ♥ by the adk-fluent crew · every <code>.build()</code> returns a real ADK object.</sub>
</p>
