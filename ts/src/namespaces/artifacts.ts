...
export class A {
  ...
  static save(
    filename: string,
    opts?: { content?: string | Uint8Array; mime?: string; metadata?: Record<string, unknown>; scope?: string },
  ): AComposite {
    return new AComposite([{
      type: "save",
      config: {
  ...
  static asJson(key: string): STransform {
    return new STransform(`A.asJson(${key})`, (state: State) => ({
      ...state,
  ...
  static asCsv(key: string, opts?: { columns?: string[] }): STransform {
    return new STransform(`A.asCsv(${key})`, (state: State) => {
      const raw = state[key];
  ...
  static asText(key: string, opts?: { encoding?: string }): STransform {
    return new STransform(`A.asText(${key})`, (state: State) => {
      const val = state[key];
  ...
  static fromJson(key: string, opts?: { indent?: number }): STransform {
    return new STransform(`A.fromJson(${key})`, (state: State) => ({
      ...state,
  ...
  static fromCsv(key: string): STransform {
    return new STransform(`A.fromCsv(${key})`, (state: State) => {
  ...
  static fromMarkdown(key: string): STransform {
    return new STransform(`A.fromMarkdown(${key})`, (state: State) => {
  ...
  static when(predicate: StatePredicate, transform: AComposite): AComposite {
    return new AComposite(transform.ops.map((op) => ({
      ...op,
  ...
  static forLlm(filename: string, opts?: { version?: number; scope?: string }): CTransform {
    return new CTransform("artifact_for_llm", {
      filename,
      version: opts?.version,
      scope: opts?.scope ?? "agent",
    }, false);
